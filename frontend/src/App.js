...
function App() {
  ...
  // NEW: The central state that links the Home view to the Edit view!
  ...
  const NavButton = ({ id, label, icon }) => {
    ...
    return (
      <button 
        onClick={() => {
          if (id === 'home' && setEditExpenseData) setEditExpenseData(null); // Clear edit state on nav
          setPage(id);
        }}
  ...
  return (
    <div className="max-w-md mx-auto h-screen flex flex-col bg-aa-gray-bg shadow-2xl relative overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto pb-24">
        {/* UPDATED: Passed editExpenseData down to the routes! */}
        ...
        {page === "group_detail" && <GroupDetail groupId={activeGroupId} setPage={setPage} currentUser={currentUser} />}
